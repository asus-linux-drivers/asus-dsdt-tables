 ...
 *
 * Disassembly of /tmp/tmp.ewjZEPARmK/ROG_Zephyrus_Duo_15_SE_GX551QS_GX551QS_e73be6176930/ROG_Zephyrus_Duo_15_SE_GX551QS_GX551QS, Mon Jun  8 14:15:03 2026
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0000D99E (55710)
 *     Revision         0x02
 *     Checksum         0xD7
 *     OEM ID           "ALASKA"
 *     OEM Table ID     "A M I "
...
DefinitionBlock ("", "DSDT", 2, "ALASKA", "A M I ", 0x01072009)
{
    ...
    Name (DTPT, Zero)
    Name (TTDP, One)
    Name (TPMB, 0xEC30F000)
    Name (TPBS, 0x4000)
    Name (TPMC, 0xEC313000)
    Name (TPCS, 0x4000)
    Name (TPMM, 0xFD210510)
    Name (FTPM, 0xFD210510)
    Name (PPIM, 0xEC54BC98)
    Name (PPIL, 0x1C)
    Name (AMDT, One)
    Name (TPMF, One)
    Name (PPIV, One)
    ...
    Name (VGAF, One)
    OperationRegion (GNVS, SystemMemory, 0xEC54B998, 0x0C)
    ...
    Scope (_SB)
    {
        OperationRegion (RAMW, SystemMemory, 0xEC539000, 0x0100)
        ...
        Device (ATKD)
        {
            ...
            Scope (\_SB.ATKD)
            {
                ...
                Method (DIAL, 1, NotSerialized)
                {
                    ...
                    {
                        DIBF = Arg0
                        Notify (^^I2CB.DIDV, One) // Device Check
                        Return (One)
                    }
                    ElseIf ((Arg0 == Zero))
                    {
            ...
            Method (WMNB, 3, Serialized)
            {
                ...
                If ((Local0 == 0x53545344))
                {
                    ...
                    If ((IIA0 == 0x00010031))
                    {
                        ...
                        While ((Local2 < 0x02))
                        {
                            If (GGOV (Zero, DerefOf (ASSS [Local2])))
                            {
                                Local0 |= (One << Local2)
                            }

                            If (GGOV (Zero, DerefOf (ASSP [Local2])))
                            {
                                Local1 |= (One << Local2)
                            }
                ...
                If ((Local0 == 0x53564544))
                {
                    ...
                    {
                        Local0 = (IIA1 & 0x0F)
                        Local1 = ((IIA1 & 0xF0) >> 0x04)
                        Local2 = Zero
                        While ((Local2 < 0x02))
                        {
                            If ((Local0 & (One << Local2)))
                            {
                                SGOV (Zero, DerefOf (ASSS [Local2]), One)
                            }
                            Else
                            {
                                SGOV (Zero, DerefOf (ASSS [Local2]), Zero)
                            }

                            If ((Local1 & (One << Local2)))
                            {
                                SGOV (Zero, DerefOf (ASSP [Local2]), One)
                            }
                            Else
                            {
                                SGOV (Zero, DerefOf (ASSP [Local2]), Zero)
                            }
    ...
    Scope (_SB.I2CB)
    {
        Device (DIDV)
        {
            ...
            Method (_CRS, 0, Serialized)  // _CRS: Current Resource Settings
            {
                ...
                Return (SBFI) /* \_SB_.I2CB.DIDV._CRS.SBFI */
            }
        }
    }
    ...
    Scope (_SB.I2CA)
    {
        Device (TPL0)
        {
            ...
            Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
            {
                ...
                CreateWordField (SBFI, \_SB.I2CA.TPL0._CRS._Y25._ADR, ADR1)  // _ADR: Address
                ADR1 = DerefOf (SADR [Zero])
