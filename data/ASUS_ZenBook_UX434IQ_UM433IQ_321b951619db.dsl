 ...
 *
 * Disassembly of /tmp/tmp.27CHi3cjOM/ZenBook_UX434IQ_UM433IQ_321b951619db/ZenBook_UX434IQ_UM433IQ, Tue Jun 30 08:48:32 2026
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0000B418 (46104)
 *     Revision         0x02
 *     Checksum         0x6A
 *     OEM ID           "_ASUS_"
...
DefinitionBlock ("", "DSDT", 2, "_ASUS_", "Notebook", 0x01072009)
{
     ...
     * required for each:
     */
    External (_SB_.PCI0.GPP0.VGA_, UnknownObj)
    ...
    Name (DTPT, Zero)
    Name (TTDP, One)
    Name (TPMB, 0xAC6EE000)
    Name (TPBS, 0x4000)
    Name (TPMC, 0xAC6F2000)
    Name (TPCS, 0x4000)
    Name (TPMM, 0xFD210510)
    Name (FTPM, 0xFD210510)
    Name (PPIM, 0xAC941F98)
    Name (PPIL, 0x1C)
    Name (AMDT, One)
    Name (TPMF, One)
    Name (PPIV, One)
    ...
    Scope (_SB)
    {
        OperationRegion (RAMW, SystemMemory, 0xAC90B000, 0x0100)
        ...
        Device (ATKD)
        {
            ...
            Scope (\_SB.ATKD)
            {
                Method (FANL, 1, Serialized)
                {
                    ...
                    If (((Arg0 == One) & (^^PCI0.SBRG.EC0.CNTD & 0x80)))
                    {
                        ^^PCI0.SBRG.EC0.CNTD &= 0x38
                        ^^PCI0.SBRG.EC0.CNTD |= 0x40
                    }
                    ElseIf (((Arg0 == Zero) & (^^PCI0.SBRG.EC0.CNTD & 0x40)))
                    {
                        ^^PCI0.SBRG.EC0.CNTD &= 0x38
    ...
    Scope (_SB.PCI0.SBRG.EC0)
    {
        ...
        Method (_QBA, 0, NotSerialized)  // _Qxx: EC Query, xx=0x00-0xFF
        {
            ...
            If (((Local0 > CNTB) && (BOTP == Zero)))
            {
                If ((Local0 == One))
                {
                    Notify (^^^GPP0.VGA, 0xD2) // Hardware-Specific
                }
                ElseIf ((Local0 == 0x02))
                {
                    Notify (^^^GPP0.VGA, 0xD3) // Hardware-Specific
                }
                ElseIf ((Local0 == 0x03))
                {
                    Notify (^^^GPP0.VGA, 0xD4) // Hardware-Specific
                }
                ElseIf ((Local0 >= 0x04))
                {
                    Notify (^^^GPP0.VGA, 0xD5) // Hardware-Specific
                }
            }
        }
        ...
        Method (_QBB, 0, NotSerialized)  // _Qxx: EC Query, xx=0x00-0xFF
        {
            ...
            If ((BOTP == Zero))
            {
                If ((Local1 == Zero))
                {
                    Notify (^^^GPP0.VGA, 0xD1) // Hardware-Specific
                }
                ElseIf ((Local1 == One))
                {
                    Notify (^^^GPP0.VGA, 0xD2) // Hardware-Specific
