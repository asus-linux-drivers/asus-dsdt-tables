 ...
 *
 * Disassembly of /tmp/tmp.cmlVGOFCtV/Zenbook_14_UM3406GA_adb0ac2b4a2a/Zenbook_14_UM3406GA, Tue May 19 18:54:51 2026
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x00011FE7 (73703)
 *     Revision         0x02
 *     Checksum         0x13
 *     OEM ID           "_ASUS_"
...
DefinitionBlock ("", "DSDT", 2, "_ASUS_", "Notebook", 0x01072009)
{
    ...
    Scope (_SB.PCI0.SBRG.EC0)
    {
        ...
        Method (_QB1, 0, NotSerialized)  // _Qxx: EC Query, xx=0x00-0xFF
        {
            ...
            Local3 = REBC (0x69, 0x02)
            Local4 = DerefOf (Local3 [Zero])
            Local5 = DerefOf (Local3 [One])
            Local4 <<= 0x08
            If ((Local5 == Zero))
            {
                If ((ACPT == One))
                {
                    Local7 = (0x2600 - Local4)
                }
        ...
        Device (ATKD)
        {
            ...
            Method (WMNB, 3, Serialized)
            {
                ...
                If ((Local0 == 0x53545344))
                {
                    ...
                    If ((IIA0 == 0x00110052))
                    {
                        Local0 = ^^PCI0.SBRG.EC0.REBC (0x64, 0x02)
                        Local1 = (0x00010000 | DerefOf (Local0 [Zero]))
                        Return (Local1)
                    }
                    ...
                    {
                        Local0 = ^^PCI0.SBRG.EC0.REBC (0x64, 0x02)
                        Local1 = (0x00010000 | DerefOf (Local0 [One]))
                        Return (Local1)
                    }
                    ...
                    If ((IIA0 == 0x00110054))
                    {
                        ...
                        Local0 = ^^PCI0.SBRG.EC0.REBC (0x65, 0x04)
                        Local1 = (DerefOf (Local0 [0x03]) & One)
                        Local2 = (DerefOf (Local0 [0x03]) & 0x02)
                        Local1 <<= 0x10
                        Local2 <<= 0x10
                        Local3 = (DerefOf (Local0 [Zero]) << 0x20)
                        Local4 = (DerefOf (Local0 [One]) << 0x30)
                        Local5 = (Local1 | DerefOf (Local0 [0x02]))
                        Local5 |= Local2
                        Local5 |= Local3
                        Local5 |= Local4
                        Local6 = (Local5 & 0xFFFFFFFF)
                        Local7 = (Local5 >> 0x20)
                        Local7 &= 0xFFFFFFFF
                        BUF0 [Zero] = Local6
                        BUF0 [One] = Local7
                        Return (BUF0) /* \_SB_.ATKD.WMNB.BUF0 */
                    }

                    If ((IIA0 == 0x00110053))
                    {
                        Local0 = ^^PCI0.SBRG.EC0.REBC (0x67, 0x02)
                        Local1 = DerefOf (Local0 [Zero])
                        Local2 = DerefOf (Local0 [One])
                        Local2 <<= 0x08
                        Local1 |= Local2
                        Local3 = ^^PCI0.SBRG.EC0.REBC (0x65, 0x04)
                        Local4 = (DerefOf (Local3 [0x03]) & One)
                        Local4 <<= 0x10
                        Local5 = (Local4 | Local1)
                        Return (Local5)
                    }
                ...
                If ((Local0 == 0x53564544))
                {
                    ...
                    If ((IIA0 == 0x00110053))
                    {
                        Name (TEMP, Buffer (One)
                        {
                             0x00                                             // .
                        })
                        TEMP [Zero] = IIA1 /* \_SB_.ATKD.WMNB.IIA1 */
                        If ((ACPT == One))
                        {
                            AL2E = 0x4E20
                            SSZE = 0x07
                        ...
                        ^^PCI0.SBRG.EC0.WEBC (0x66, One, TEMP)
                        Return (One)
                    }
