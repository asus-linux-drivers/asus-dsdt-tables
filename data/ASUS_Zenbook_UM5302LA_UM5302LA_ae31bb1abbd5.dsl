 ...
 *
 * Disassembly of /tmp/tmp.rmXKcNmsP5/Zenbook_UM5302LA_UM5302LA_ae31bb1abbd5/Zenbook_UM5302LA_UM5302LA, Tue May 26 16:56:35 2026
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0000CE31 (52785)
 *     Revision         0x02
 *     Checksum         0xD7
 *     OEM ID           "_ASUS_"
...
DefinitionBlock ("", "DSDT", 2, "_ASUS_", "Notebook", 0x01072009)
{
    ...
    Name (VGAF, One)
    OperationRegion (GNVS, SystemMemory, 0x96158D18, 0x0D)
    ...
    Method (_WAK, 1, NotSerialized)  // _WAK: Wake
    {
        ...
        SWAK (Arg0)
        XWAK (Arg0)
        \_SB.PCI0.SBRG.EC0.ACAD.ACDC = 0xFF
        Return (WAKP) /* \WAKP */
    }
    ...
    OperationRegion (EXBU, SystemMemory, 0x9616B698, 0x0252)
    ...
    Scope (_SB.PCI0.SBRG)
    {
        ...
        Device (EC0)
        {
            ...
            Method (UPHK, 1, Serialized)
            {
                ...
                If (((Arg0 >= 0x02) && (Arg0 <= 0x08)))
                {
                    Switch (Arg0)
                    {
                        Case (0x03)
                        {
                            ECWT (0xAA, RefOf (MSFG))
                        }
                        Case (0x04)
                        {
                            ECWT (Zero, RefOf (MSFG))
                        }
                        Case (0x05)
                        {
                        }
            ...
            Method (MPTC, 0, Serialized)
            {
                Name (ATCS, Buffer (0x2A) {})
                CreateWordField (ATCS, Zero, SSZE)
                CreateByteField (ATCS, 0x02, FPPT)
                CreateDWordField (ATCS, 0x03, SMU0)
                CreateByteField (ATCS, 0x07, SPPT)
                CreateDWordField (ATCS, 0x08, SMU1)
                CreateByteField (ATCS, 0x0C, SPTT)
                CreateDWordField (ATCS, 0x0D, SMU2)
                CreateByteField (ATCS, 0x11, THLT)
                CreateDWordField (ATCS, 0x12, SMU3)
                CreateByteField (ATCS, 0x16, EDCV)
                CreateDWordField (ATCS, 0x17, SMU4)
                CreateByteField (ATCS, 0x1B, P3TW)
                CreateDWordField (ATCS, 0x1C, SMU5)
                CreateByteField (ATCS, 0x20, STLA)
                CreateDWordField (ATCS, 0x21, SMU6)
                CreateByteField (ATCS, 0x25, SMLT)
                CreateDWordField (ATCS, 0x26, SMU7)
                SSZE = 0x2A
                FPPT = 0x06
                SPPT = 0x07
                SPTT = 0x08
                ...
                STLA = 0x22
                SMLT = 0x2E
                Name (FSMT, Buffer (0x11) {})
                CreateWordField (FSMT, Zero, SIZE)
                CreateByteField (FSMT, 0x02, FRPT)
                CreateDWordField (FSMT, 0x03, VAL0)
                CreateByteField (FSMT, 0x07, SLPT)
                CreateDWordField (FSMT, 0x08, VAL1)
                CreateByteField (FSMT, 0x0C, SMMT)
                CreateDWordField (FSMT, 0x0D, VAL2)
                SIZE = 0x11
                FRPT = 0x06
                SLPT = 0x07
                SMMT = 0x2E
                Name (FSST, Buffer (0x07) {})
                CreateWordField (FSST, Zero, SIEE)
                CreateByteField (FSST, 0x02, FRPP)
                CreateDWordField (FSST, 0x03, DAT0)
                SIEE = 0x07
                FRPP = 0x06
    ...
    Scope (_SB.I2CA)
    {
        ...
        Device (SPKR)
        {
            ...
            Name (_SUB, "10431A83")  // _SUB: Subsystem ID
            Name (_UID, One)  // _UID: Unique ID
            Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
            {
                Name (RBUF, ResourceTemplate ()
        ...
        Scope (GPP6)
        {
            ...
            Device (WLAN)
            {
                ...
                Method (MTCL, 0, Serialized)
                {
                    ...
                    Return (SAR) /* \_SB_.PCI0.GPP6.WLAN.MTCL.SAR_ */
                }

                Method (MTDS, 0, Serialized)
                {
                    Name (SAR, Package (0x1F)
                    {
                        0x4D, 
                        0x54, 
                        0x44, 
                    ...
                    Return (SAR) /* \_SB_.PCI0.GPP6.WLAN.MTDS.SAR_ */
                }

                Method (MTGS, 0, Serialized)
                {
                    Name (SAR, Package (0x1C)
                    {
                        0x4D, 
                        0x54, 
                        0x47, 
                    ...
                    Return (SAR) /* \_SB_.PCI0.GPP6.WLAN.MTGS.SAR_ */
                }
            }
        }
    }
